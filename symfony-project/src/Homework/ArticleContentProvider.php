...
class ArticleContentProvider implements ArticleContentProviderInterface
{
     ...
     */
    public function __construct(private bool $isStyleBold, private PasteWords $pasteWords)
    {
    }
    ...
    public function get(int $paragraphs, string $word = null, int $wordsCount = 0): string
    {
        ...
        if ($word && $wordsCount) {
            ...
            $generatedParagraphs = $this->pasteWords->paste($generatedParagraphs, $formattedWord, $wordsCount);
        }
    ...
    {
        $paragraphs = [
            '"Courage wasn\'t a matter of taking the whole mountain in a single massive leap. Courage was taking it one step at a time, doing what was necessary now, preparing for the next step, and refusing to worry about whether some step in the future would be the one that would break him" - *Kell Tainer*.'
            ,
            '"Anger always makes you feel good at the time. Makes you feel bigger than yourself, and makes you feel that everything you do is justified. But it\'s a trap" - *Corran Horn*.'
            ,
